...
int takenumber(long long int ID, int flag, int initial, int number) {
   char id_str[20]; // ID'yi tutacak bir karakter dizisi oluþturuyoruz, 20 karakter yeterlidir varsayalým.
    snprintf(id_str, sizeof(id_str), "%lld", ID); // ID'yi stringe dönüþtürüyoruz.

    // flag kontrolü
    if (flag == 0) {
        // initial deðeri ID stringindeki bir indexi temsil ediyor
        // number deðeri ise bu indexten sonraki kaç indexin alýnacaðýný belirtiyor
        int id_length = strlen(id_str);
        if (initial < id_length) { // initial deðeri ID stringinin uzunluðundan küçük olduðunda devam ediyoruz
            ...
            return atoi(result); // Sonuç karakter dizisini tamsayýya dönüþtürüyoruz ve döndürüyoruz
    	} 
	}
	
	if (flag == 1) {
        // initial deðeri ters yönde ID stringindeki bir indexi temsil ediyor
        // number deðeri ise bu indexten önceki kaç indexin alýnacaðýný belirtiyor
        int id_length = strlen(id_str);
        if (initial < id_length) { // initial deðeri ID stringinin uzunluðundan küçük olduðunda devam ediyoruz
            char result[20]; // Sonuç karakter dizisi
            int i;
